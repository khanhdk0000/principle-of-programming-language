...
class Emitter():
    ...
    def getJVMType(self, inType):
        ...
        if typeIn is cgen.IntType:
            ...
            return "(" + "".join(list(map(lambda x: self.getJVMType(x), inType.intype))) + ")" + self.getJVMType(
                inType.restype)
        elif typeIn is cgen.FunctionType:
            return "(" + "".join(list(map(lambda x: self.getJVMType(x.mtype), inType.param))) + ")" + self.getJVMType(
                inType.restype)
    ...
    *   @param in the type descriptor of the method.
    ...
    def emitINVOKESTATIC(self, lexeme, in_, frame):
        ...
        if not type(typ.restype) is cgen.VoidType:
            frame.push()
    ...
    *   generate ior
    ...
    def emitREOP(self, op, in_, frame):
        ...
        is_float = type(in_) is cgen.FloatType
