...
class TestCodeGen():
    ...
    def test(input, expect, num):
        ...
        try:
            ...
            subprocess.call("java  -jar "+ JASMIN_JAR + " " + path + "/BKITClass.j",shell=True,stderr=subprocess.STDOUT)
            

            f = open(solpath + str(num) + ".txt","w")
            subprocess.call("java -cp ./lib" + os.pathsep + ". BKITClass",shell=True, stdout = f)
            f.close()
        except StaticError as e:
            dest.write(str(e))
