    ...
    def visitVar(self, ctx: BKITParser.VarContext):
        if ctx.scalar():
            return VarDecl(ctx.scalar().accept(self), [],
                           ctx.scalar_value().accept(self)) if ctx.scalar_value() else VarDecl(
                ctx.scalar().accept(self), [], None)
        else:
            var = ctx.composite().accept(self)
            return VarDecl(var[0], var[1], ctx.composite_value().accept(self)) if ctx.composite_value() else VarDecl(
                var[0], var[1], None)
